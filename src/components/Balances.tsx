...
const Balances: React.FC<BalancesProps> = ({
    ...
    const rows = balances.filter((row: any) => row.WalletBalance > 0 && row.PossibleSpam === false).map((item: any, index: number) => {
        return {
            ...
            TokenPrice: item.TokenPrice ? Number(item.TokenPrice) : 0,
            WalletBalance: item.WalletBalance ? Number(item.WalletBalance) : 0,
            USDValue: item.USDValue ? Number(item.USDValue) : 0,
